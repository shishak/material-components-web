...
export default class MDCToolbarFoundation extends MDCFoundation {
  ...
  setKeyHeights_() {
    ...
    const newToolbarRowHeight = this.adapter_.getViewportWidth() <= breakpoint ?
      MDCToolbarFoundation.numbers.TOOLBAR_ROW_MOBILE_HEIGHT : MDCToolbarFoundation.numbers.TOOLBAR_ROW_HEIGHT;

    if (newToolbarRowHeight !== this.keyHeights.toolbarRowHeight) {
      const isInit = this.keyHeights.toolbarRowHeight === 0;
      this.keyHeights.toolbarRowHeight = newToolbarRowHeight;

      if (isInit) {
        const flexibleRowMaxRatio =
          this.adapter_.getFlexibleRowElementOffsetHeight() / this.keyHeights.toolbarRowHeight;
        ...
        this.keyHeights.maxTranslateYRatio =
          this.fixedLastrow_? this.keyHeights.toolbarRatio - flexibleRowMaxRatio : 0;
        this.keyHeights.scrollThesholdRatio =
          (this.fixedLastrow_? this.keyHeights.toolbarRatio : flexibleRowMaxRatio) - 1;
      }
  ...
  scrolledOutOfTheshold(scrollTop) {
    if (scrollTop > this.keyHeights.scrollTheshold) {
      if (this.executedLastChange) {
        return true;
      }
      this.executedLastChange = true;
    } else {
      this.executedLastChange = false;
    }
    return false;
  ...
  changeToolbarStyles() {
    ...
    this.scrollFrame_ = requestAnimationFrame(() => {
      ...
      if (this.scrolledOutOfTheshold(scrollTop)) {
